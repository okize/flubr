...
module.exports = {
  ...
  create(req, res)  {
    ...
    return twitter.get('users/show', {screen_name: req.body.user}, function(err, data) {
      ...
      if ((err != null) && (err.code === 34)) {
        ...
        return User.find({userid: user.userid}, function(err, results) {
          if (err) { throw err; }
          if (!results.length) {
  ...
  update(req, res) {
    return User.findByIdAndUpdate(req.params.id, { $set: req.body }, function(err, results) {
      if (err != null) { res.send(500, {error: err}); }
  ...
  // deletes user record
  delete(req, res) {
    return User.findOneAndRemove({userid: req.params.id}, function(err, results) {
      if (err != null) { res.send(500, {error: err}); }
      if (results != null) {
